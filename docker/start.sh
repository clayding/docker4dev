...
echo -e "\033[32m[Toolchaindir]:$toolchain_dir \033[0m"
...
if [ "$git_str" == "" ];then
    ...
    echo -e "\033[32m[Kernelurl]:$kernel_url\033[0m"
    echo -e "\033[32m[Kernelbranch]:$kernel_branch\033[0m"
    echo -e "\033[32m[Kerneldir]:$source_dir/$kernel_dir\033[0m"
    if [ $kernel_url ];then
        tmp_path=$PWD
        echo -e "\033[34mClone the kernel source from $kernel_url\033[0m"
...
if [ "$git_str" == "" ];then
    ...
    echo -e "\033[32m[Ubooturl]:$u_boot_url\033[0m"
    echo -e "\033[32m[Ubootbranch]:$u_boot_branch\033[0m"
    echo -e "\033[32m[Ubootdir]:$source_dir/$u_boot_dir\033[0m"
    if [ $u_boot_url ];then
        tmp_path=$PWD
        echo -e "\033[34mClone the uboot source from $u_boot_url\033[0m"
